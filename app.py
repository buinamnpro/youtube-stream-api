...
def get_audio_url():
    ...
    print(f"[get_audio_url] title='{title}', artist='{artist}', url='{youtube_url}'")
    ...
    print(f"[get_audio_url] Token {token} -> {audio_url}")
...
def stream_audio():
    ...
    print(f"[stream] bắt đầu tải MP3 cho URL: {youtube_url}")

    def generate():
        print("-> Bắt đầu stream MP3 tạm thời...")
        mp3_path, temp_dir = download_mp3_to_temp(youtube_url)
        if not mp3_path:
            print("[stream] Không tạo được file MP3 tạm")
            yield b""
            return
        try:
...
def stream_mp3_token(token):
    ...
    print(f"[stream_mp3_token] Nhận token {token}, URL = {youtube_url}")
